...
export type CumulativeEvent =
  ...
  | { type: 'jailed' }
  | { type: 'vehicle_abandoned' }
  | { type: 'vehicle_broke_down' }
 ...
 *  Initial deal always draws easy-tier cards only — harder tiers are put back. */
export async function dealInitialMissions(
  ...
  for (const playerId of humanPlayerIds) {
    await drawMission(db, gameId, playerId, season, 'easy')
    await drawMission(db, gameId, playerId, season, 'easy')
  }
}
 ...
 *  Pass tierFilter to only draw cards of that tier (others are skipped back to discard). */
export async function drawMission(
  ...
  // Count cities on this board so we can skip cards with impossible targets
  const cityCountRow = await db.prepare(
    `SELECT COUNT(*) AS cnt FROM game_cities WHERE game_id = ?`
  ).bind(gameId).first<{ cnt: number }>()
  const cityCount = cityCountRow?.cnt ?? 999

  // Draw cards until one is playable on this board, skipping unachievable ones
  let cardId: number | null = null
  const maxAttempts = deck.length + discard.length
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const card = getMissionCard(candidate)

    // Skip cards that don't match the tier filter (put them back in discard)
    if (tierFilter && card?.tier !== tierFilter) {
      discard.push(candidate)
      continue
    }

    // Skip cities_visited cards whose target exceeds the number of cities on this board
    if (card?.objectiveType === 'cities_visited' && Number(card.params.target) > cityCount) {
      discard.push(candidate)
      continue
    }
...
function evaluateObjective(card: MissionCard, snapshot: MissionSnapshot, progress: Record<string, unknown>): boolean {
  ...
  switch (card.objectiveType) {
    ...
    case 'times_jailed':
      return (Number(progress.times_jailed) || 0) >= target
    case 'vehicles_abandoned':
      return (Number(progress.vehicles_abandoned) || 0) >= target
    case 'vehicles_broken_down':
      return (Number(progress.vehicles_broken_down) || 0) >= target
    case 'vehicle_stationary_gte':
      return (snapshot.maxVehicleStationary ?? 0) >= target
...
export async function updateCumulativeProgress(
  ...
  const cumulativeTypes: MissionObjectiveType[] = [
    ...
    'times_jailed', 'vehicles_abandoned', 'vehicles_broken_down',
  ]
  ...
  for (const row of held) {
    ...
    if (event.type === 'sold_units' && (card.objectiveType === 'total_sold_units' || card.objectiveType === 'sold_units_of_type')) {
      ...
      updated = true
    } else if (event.type === 'jailed' && card.objectiveType === 'times_jailed') {
      progress.times_jailed = (Number(progress.times_jailed) || 0) + 1
      updated = true
    } else if (event.type === 'vehicle_abandoned' && card.objectiveType === 'vehicles_abandoned') {
      progress.vehicles_abandoned = (Number(progress.vehicles_abandoned) || 0) + 1
      updated = true
    } else if (event.type === 'vehicle_broke_down' && card.objectiveType === 'vehicles_broken_down') {
      progress.vehicles_broken_down = (Number(progress.vehicles_broken_down) || 0) + 1
      updated = true
    }
